...
class APIClient:
    ...
    def post(self, endpoint, payload):
        url = f"{self.base_url}{endpoint}"
        return requests.post(url, json=payload, headers=self.headers, timeout=5)

    def get_user(self, user_id, test_type=None):
        response = self.get(f"/api/users/{user_id}")
        print(f"GET /api/users/{user_id} - Status Code: {response.status_code}")  # Debugging statement
